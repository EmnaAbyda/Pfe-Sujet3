...
public observer   :  Subscriber<{}>;
  ...
  public getItems(): Observable<CartItem[]> {
   ...
   // Add to cart
   public addToCart(product: Produit, quantity: number) {
    let message, status;
...
public calculateStockCounts(product: CartItem, quantity): CartItem | Boolean {
  ...
  let stock = product.product.prix;//stock=prix
  if(stock < qty) {
...
public getTotalAmount(): Observable<number> {
  return this.cartItems.pipe(map((product: CartItem[]) => {
    return products.reduce((prev, curr: CartItem) => {
      return prev + parseInt(curr.product.prix) * curr.quantity;
    }, 0);
  }));
}

// Update Cart Value
public updateCartQuantity(product: Produit, quantity: number): CartItem | boolean {
